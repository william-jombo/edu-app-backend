<?php
// backend/api/students/get_exam_review.php
...
try {
    ...
    // Get submission
    $subStmt = $db->prepare("
        SELECT id, mcq_score, written_score, grading_status
    ...
    // Get MCQ answers (stored as single letters)
    $mcqStmt = $db->prepare("
        SELECT question_id, answer_text AS student_answer
        FROM exam_written_answers
        WHERE submission_id = :submission_id
    ");
    $mcqStmt->execute([':submission_id' => $submission_id]);
    $mcqAnswers = [];
    while ($row = $mcqStmt->fetch(PDO::FETCH_ASSOC)) {
        $mcqAnswers[$row['question_id']] = $row['student_answer'];
    }

    // Get written answers with grading
    $waStmt = $db->prepare("
        SELECT 
            question_id,
            answer_text,
    ...
    while ($row = $waStmt->fetch(PDO::FETCH_ASSOC)) {
        $writtenAnswers[$row['question_id']] = [
    ...
    foreach ($questions as $q) {
        if ($q['question_type'] === 'mcq') {
            $student_answers[$q['id']] = $mcqAnswers[$q['id']] ?? null;
        } else {
            $student_answers[$q['id']] = $writtenAnswers[$q['id']] ?? null;
        }
    }
